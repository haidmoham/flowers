(() => {
  ...
  const fragment = `
    ...
    void main() {
      ...
      // The photographed fibres carry the light. The substrate stays quiet.
      float luminance = dot(source, vec3(.25, .6, .15));
      float subject = smoothstep(.05, .16, luminance);
      float presence = mix(.08, 1., subject);
  ...
  function render() {
    if (!ready) return;
    gl.uniform1f(uTime, reduced.matches ? 20 : elapsed);
